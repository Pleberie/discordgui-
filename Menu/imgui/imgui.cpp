...
#pragma clang diagnostic ignored "-Wformat-nonliteral"      // warning : format string is not a string literal              // passing non-literal to vsnformat(). yes, user passing incorrect format  can crash the code.
...
void ImGui::EndFrame()
{
  ...
  //  IM_ASSERT(g.CurrentWindowStack.Size == 10);    // Mismatched Begin()/End() calls
...
bool ImGui::ButtonT(const char* label, ImU32 clr, const ImVec2& size_arg, bool active) {
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	ImGuiWindow* window = GetCurrentWindow();
	if (active)
		RenderFrame(window->DC.CursorPos, ImVec2(window->DC.CursorPos.x + size_arg.x, window->DC.CursorPos.y + size_arg.y), clr, true, style.FrameRounding);
...
bool ImGui::TabEx(const char* label, const ImVec2& size_arg, bool enable, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	// Render
	const ImU32 col = GetColorU32((hovered && held) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
	window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(1, 110, 166, 220), ImColor(39, 41, 61, 220), ImColor(39, 41, 61, 220), ImColor(1, 110, 166, 220));
	if (enable) window->DrawList->AddRectFilled(bb.Min, bb.Min + ImVec2(5, 40), ImColor(0, 122, 206, 255));
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::Tab(const char* label, bool enable, const ImVec2& size_arg)
{
	return TabEx(label, size_arg, enable, 0);
}
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	const float asofijs = frame_bb.Max.y; //wtfxd
	if (!is_multiline) {
		RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Max.y - 2), frame_bb.Max, ImColor(255, 255, 255, 100), false, 0);
		RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
	}
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	return InputTextEx(label, buf, (int)buf_size, ImVec2(425, 30), flags, callback, user_data);
}
